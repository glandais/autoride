...
**Project**: AutoRide - Automatic bike trip detection from motion sensors and GPS (the ML
half is planned, not built — see **ML** below)
...
- **ML**: none. `tflite_flutter` was declared from T001 and never imported, and was removed
  on 2026-09-13; T016-T019 will add whichever runtime the classifier actually needs
...
**A car is not refused by any of this, and nothing else refuses one either (T051, T053).**
Replayed over a real ride and over a drive to the shops, the windowed fit scores 0.331 and
0.298 — indistinguishable. `VehicleSpeedWatch` accumulates the speed evidence, on
**provider-measured** speeds only (a derived `dsp` inherits its fixes' accuracy and reads
55 km/h on a night ride) — but since T053 it **decides nothing**: it sets
`Trip.suspectedVehicle`, History paints a badge, and the ride is kept. It held that power
from T051 (2026-09-07) to T053 (2026-09-10), and on 2026-09-09 it deleted 71.8 km of real
rides, one of them cut into seven fragments (L-106, L-107). The thresholds over-flag a fast
rider by construction and are **not to be recalibrated**: the mode of travel is to be
classified from the IMU, not from a speed — see `vehicle_speed_watch.dart`.
...
**And it fires on a pocket (T054).** Over 26 h on 1.0.0+16 the windowed fit opened 26 trips and
kept 4: an evening at rest produced 22 phantom recordings. The corroboration is three consecutive
...
`tasks/T050-windowed-motion-fit.md` and ledger §9; the vehicle veto, its corrected
calibration and its demotion to a badge: `tasks/T051-vehicle-veto.md`,
`tasks/T053-vehicle-evidence-not-verdict.md` and ledger §10. Thresholds: `lib/core/constants/app_constants.dart`.
