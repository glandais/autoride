...
## Phase 11.7: A descent is not a car, and a walk is not a ride (T052)

2026-09-09, two bicycle commutes on **1.0.0+14** — the first build carrying T051 — and a working
day between them. The evening commute is a single clean recording (8 803 m, `avg` 19.7,
`vfx: 0 / vmf: 38`, one pause, an automatic stop) and T050's departure took four seconds. The
morning commute was **cut in two and its first 1 561 m deleted**, because a descent at 39.9 km/h
tripped the vehicle veto — which is exactly the cost §10 wrote down in advance and asked the next
ride to test. Diagnoses in ledger §11 (L-102, L-103, L-104, L-105).

- ⏳ **T052**: A Descent Is Not a Car, and a Walk Is Not a Ride
  - *Status*: **the three changes are written and green (806 tests); the device run is outstanding** — the next pair of commutes, per §4.
  - *Detail*: `tasks/T052-veto-and-walks.md`; ledger L-102, L-103, L-104, L-105
  - *Scope*: **(1)** The veto's **live** arm measures the wrong thing: the descent is 6 measured fixes ≥ 35 km/h over 10 s, max **39.9**, `ac` 4.3–14 m — §10's drive is 5 over 8 s, max **39.3**. The cyclist is faster and holds it longer, so no (`vehicleSpeedKmh`, `vehicleSustainSeconds`) pair separates them. Proposed: a new `vehicleLiveSpeedKmh` (**50**) for `isVehicleNow` alone, leaving `vehicleSpeedKmh` (35) to the **end-of-ride share arm**, which was right about this morning and never got to answer — **0.0 % / 4.7 % / 14.3 % on three rides against 27.0 % on the drive**, with `vehicleSpeedMinShare` already at 0.25 in the gap (L-102). **(2)** `Trip.discardReason`'s `avg` arm gains a distance proof — `minTripLoopDistanceMeters` (**1 000 m**) — because that arm exists for the loop ride and a loop ride has distance: two walks are in History as cycling rides at 545 m / `net` 52 and 219 m / `net` 45, and replayed over all 40 recordings exactly those two verdicts change (L-103). **(3)** A **no-progress deadline** (`noProgressStopTimeout`, 420 s) ends a recording that has neither seen a *measured* cycling speed nor travelled `minTripNet` — the start stays permissive, so T048/T050's four-second departure is untouched. Both terms are load-bearing: timing alone cannot carry it (one genuine commute waited **423 s** for its first measured cycling speed), and at 420 s that same ride had already covered **271 m** against the false recordings' 0-63 m. Over the corpus: 11 recordings cut, **~62 min of GPS recovered**, no genuine ride touched (L-103, L-105).
  - *Evidence*: truncating the ride at the veto leaves `vfx 4 / vmf 11` = 36 % — **the live arm destroys the evidence the end-of-ride arm needs**. Every false start has `spk 0` and `vt` absent (motion-only), and `gav` is *higher* on foot (1.4–3.8) than on the commute (0.77–1.84). Held in reserve, measured and not implemented: the motion **duty cycle** separates the corpus cleanly — median `win.sd` **0.86 / 1.38 / 2.28** on the real rides against **0.01–0.22** on all thirteen false ones (L-104).
  - *Dependencies*: T051

## Phase 11.8: A veto that records instead of deleting (T053)

2026-09-09 evening and the night after, three sporting rides on **1.0.0+15** — the first build
carrying T052 — recorded in parallel on a Karoo. T052's deadline and its loop-distance proof both
hold; its **vehicle threshold does not**. AutoRide tracked all three rides correctly (distances
within 0.6-4 % of the FIT, peaks within 0.3 km/h) and then **deleted two of them, 71.8 km**, the
64.5 km one after cutting it into seven fragments. Diagnoses in ledger §12 (L-106, L-107).

- ⏳ **T053**: A Veto That Records Instead of Deleting
  - *Status*: **written and green (810 tests); the device run is outstanding** — the next sporting ride, per §4.
  - *Detail*: `tasks/T053-vehicle-evidence-not-verdict.md`; ledger L-106, L-107
  - *Scope*: **(1)** `Trip` gains `suspectedVehicle` (schema v4); `discardReason` loses its `vehicle` arm; `_watchForVehicle` stops ending recordings; History shows a badge so a real drive is one tap to delete (L-106). **(2)** `vehicleLiveSpeedKmh`, `vehicleSustainSeconds`, `vehicleSpeedWindowFixes`, `VehicleSpeedWatch.isVehicleNow`/`hasFired` and `vehicleCooldownPeriodSeconds` are deleted with the live arm; `vehicleSpeedKmh` and `vehicleSpeedMinShare` survive as what produces the **flag**. **(3)** L-107 — the cooldown was armed only by the end-of-ride arm, never the live one, because `_watchForVehicle` bypasses `_finalizeAndStopTrip`; that is why one ride became seven fragments 21-26 s apart, and it goes away with the live arm.
  - *Evidence*: **the axis is inverted, not mis-thresholded.** The 2026-09-07 town drive has the **lowest maximum in the corpus** (39.3 km/h) and a 27.0 % share indistinguishable from sporting ride #1's 27.7 %; the 64.5 km ride reads **55.5 %** aggregated with 1.8 % of samples above the 50 km/h live threshold T052 had just introduced. No ordering by measured speed separates them.
  - *Direction*: **the speed axis is closed by decision as well as by evidence.** What will tell these modes apart is the **inertial unit** — the T034 capture feeding a classifier. `vehicleSpeedKmh` and `vehicleSpeedMinShare` are not to be recalibrated again.
  - *Dependencies*: T052
...
## Phase 11.9: Five seconds of a pocket is not a ride (T054)

2026-09-12/13, 26 h on **1.0.0+16** — the first build carrying T053 — cross-referenced against two
Karoo FIT files. T053, T052's deadline and T046's iOS survival all hold; what does not is the
**start path**. The app opened **26 trips and kept 4**: an evening at rest produced **22 phantom
recordings**, 3 h 44 of them, and the one 41 km ride it measured to within 66 m it also started
22 min late and ended 11 min late. Diagnoses in ledger §13 (L-108 … L-112).

- ⏳ **T054**: The Corroboration Window, and the Axis That Never Votes
  - *Status*: **diagnosis + the §4 replay, which refuted the plan.** Nothing written in `lib/`; the harness is `scripts/t054-replay/`. Run over **seven pooled `sv:3` corpora** (09-07 → 09-13, +13 → +16, start-path constants verified identical): 299 890 evaluations, 83.6 h, 21 real journeys, 72 phantom recordings. **No candidate keeps the departures and loses the false starts** — the best that still start all 21 journeys cut them 38–45 % for 1–2 min of median added latency (L-114).
  - *Detail*: `tasks/T054-start-corroboration.md`; ledger L-108, L-109, L-110, L-111, L-112
  - *Scope*: **(1)** The corroboration is 3 consecutive seconds and rest supplies 3 consecutive seconds (L-108) — but **lengthening it is refuted** (L-114): a ride's own median burst is **two seconds**, so a long streak refuses the rider before the pocket (`streak 30s` starts 11 of 21 journeys). What is left is the **duty cycle** (L-104): `duty ≥ 28 %/60 s` or `≥ 18 %/120 s`, both keeping 21/21 journeys, for ~40 % fewer false starts and 1–2 min of latency. **Order matters: (3) first** — that latency is only acceptable back-dated away. **(2)** Make the speed weight honest: `tripStartSpeedWeight` 0.4 has never voted, so either delete it and set the threshold on the motion score explicitly, or open the gate to decide, or say plainly that `noProgressStopTimeout` is the speed test (L-109). **(3)** The riding-tail cut must accept a **derived** speed — `dsp` exists on these fixes, `sp` does not — or no iPhone ride is ever back-dated and every start is late by however long (1) adds (L-110). **(4)** `resumeMovementThresholdSeconds` (5) inherits whatever (1) settles on (L-111). **(5)** `sensorSamplingRateMedium` asks iOS for 40 Hz and gets 24 (L-112).
  - *Evidence*: **riding and standing still are the same signal at different densities.** Duty cycle above the threshold **23.4 % riding vs 7.0 % at rest**, but burst length median **2 s vs 2 s**, p90 11 s vs 9 s — only the density separates (L-114). And the detector is a max-statistic over hours of rest: `win.sd` at rest reads p50 **0.47** / p90 2.13 / p99 **5.22** against the two real rides' p50 **1.71** and **3.46** — medians 4–7× apart, tails fully overlapping — and 31 700 s of evening puts >300 s above that p99. Every false start is ~18 s of quiet then a five-second burst (22:12:28→33: `asd` 2.667 → 3.512, `gav` 1.538 → 1.306, `c` 0.834 → 1.000). `vt:false` on **26 of 26** starts and on 99.55 % of 55 269 evaluations, because the GPS gate is *closed* at the instant of decision (`gate {a:"sched", why:"stationary"}` eleven seconds before trip 133) and fully open five minutes later. **No `bdate` in 26 h**: `buf {a:"tail", n:3, kp:0}`, because iOS reports `sp:0` right through a departure.
  - *Cost*: **8.1 h of GPS gate open out of 26**, against ~3.2 h of riding; 55 % battery at 20:16 → **1 % at 11:06** with no ride in between. The file is verbose, so that is an upper bound with the log's own cost inside it, not a clean T041 item 4 measurement.
  - *Direction*: unchanged and reinforced. This buys a start path that costs less and lies less; it classifies nothing. **L-100 now has a third face** — motion cannot separate a bicycle from a car (§10), from a walk (§11), or from a phone being picked up (§13) — and the classifier remains the **inertial unit**, T034's capture, which still has no task.
  - *Blocks*: T041 item 11 (fails on this build, via L-110)
  - *Dependencies*: T050 (the windowed fit this extends), T053 (the build that produced the log)
  - *Acceptance*: ~~replay first~~ **done** — the bar to beat is **21/21 journeys at 15.6 false starts a day**, re-measurable on any corpus with `scripts/t054-replay/`. The 2026-09-03 kitchen logs are `sv:2` and cannot be replayed at all. Then, on a device: an idle evening (T049 §5 run 1, never yet passed on any build — and it will **not** pass now, ~40 % is the ceiling, so the criterion has to become a rate, not zero) and a Karoo-paired ride starting within 60 s **with `bdate` present**.
...
- ⏳ **T043**: Opt-in Audit Log
  ...
  - *Decisions (maintainer, 2026-09-02)*: NDJSON gzipped · share sheet only, no backend · two levels (normal/verbose, never raw 50 Hz sensors) · circular buffer 7 days / 200 000 rows / ~20 MB — **revised 2026-09-13 to 7 days / 1 200 000 rows / 100 MB** (L-113: at a measured 87 B/line the row bound had always bitten first and the byte bound had never fired, so the log covered ~26 h instead of the intended week)
  - *Implementation*:
