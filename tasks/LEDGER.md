...
| 14 — a trip cannot outlive its GPS, and a finished trip resets the detectors | (this change) | **L-074** (new) | The coordinator now runs a **GPS-loss watchdog** on its existing 1 Hz supervisor: a recording (active *or* paused) trip with no fix for `AppConstants.gpsLossStopTimeout` (**10 min**) is finalized through the same `_finalizeAndStopTrip` path as an automatic stop, with an explicit warning log. The reference instant is the *reception* time of the last fix, or the trip's own start while none has arrived yet, so a slow first fix cannot end a ride. Two detector-reset bugs closed in the same file: `TripStartDetector.reset()` was called only on the `startRecording` **error** path, so a finished trip left `consecutiveDetections ≥ 3` in place and a single strong sample arriving within `tripStartDetectionWindowSeconds` of the last positive detection started the next trip on **one** sample (the streak rule silently defeated after every ride); and `TripStopDetector.reset()` is now called when a trip *starts*, not only when one ends. `TripRecorderService.stopRecording()` returns `Future<Trip?>` and logs a warning instead of throwing `StateError` when nothing is recording. Tests 423 → 430 (+7 coordinator tests, each mutation-checked: removing the watchdog tick or either detector reset fails them). || 15 — detection stops going blind for half the time | (this change) | **L-075** (new) | The trip-start path had a ~50 % duty cycle. `Detecting` was entered on the **first motion sample of any kind** (`_onMotionData`'s idle branch called `startDetecting()` before it had asked the detector anything), so a merely-carried phone sat in `Detecting` permanently; 30 s later `_checkDetectionTimeout` fired and did three things — returned to idle, **armed `tripStartCooldownPeriodSeconds` (30 s) of cooldown**, during which `analyzeForTripStart` returns `false` before computing anything, and tore the motion + GPS subscriptions down to restart them 100 ms later. The next motion sample re-entered `Detecting` and the cycle repeated for as long as anything moved the phone, so a real departure landing in a cooldown was missed for up to half a minute, over and over. Three changes: (1) the cooldown is **removed from the timeout path** — nothing happened there, no trip was started, so there is nothing to back off from; (2) the cooldown is **moved to where a false start actually occurs** — `_finalizeAndStopTrip` now reads the `Trip` returned by `stopRecording()` and arms it when `status == TripStatus.discarded`, i.e. a trip that really started and was thrown away for being under `minTripDurationSeconds` (L-068); no cooldown was applied there before, which is the case that most deserved one; (3) `Detecting` is entered by `_analyzeForTripStart` only once the detector has **counted a detection** (`consecutiveDetections > 0`), so the phase means what the UI says it means ("Analyzing motion…") and a carried phone no longer generates a timeout every 30 s. The timeout itself stays at 30 s but now costs exactly one streak reset — no cooldown, no stream teardown, so detection is never interrupted. The confirmation rule (3 detections, ≤ 1/s, 5 s window) and `TripStartDetector`'s score formula are untouched. Also in this change: `wakelock_plus` **removed** from `pubspec.yaml` (zero call sites in `lib/`; its Android implementation is `FLAG_KEEP_SCREEN_ON` on the *activity window* and its iOS one `UIApplication.isIdleTimerDisabled` — a **screen** wake lock that does nothing without a foregrounded activity, so it could never have helped background sensor delivery, which `flutter_background_service_android`'s real `PARTIAL_WAKE_LOCK` already covers since L-067), and five constants with no consumer and an explicit `TODO(T041)` deleted: `movementThreshold`, `tripStartGpsGracePeriodSeconds`, `onboardingPageCount`, `onboardingAnimationDuration`, `onboardingPageTransitionDuration`. Tests 430 → 448. |
...
| 19 — a verbose log outlives its own session | (this change) | **L-085**, **L-086** (both closed) | T047. Three audit emits ran on the 50 Hz motion path — the stationary window verdict (`win`), the stop decision (`stop`) and the resume evaluation (`res`) — 136 674 of the 151 212 lines of the 2026-09-02 Pixel file, 90 % of it, for 26 minutes of recording, and enough for the 20 MB byte bound to silently delete that file's own launch header, `sess start`, `fgs` and `perm` lines. All three now carry the throttle `07fabee` gave `start`: one line per `detectionEvaluationInterval` (`win` per `auditWindowVerdictInterval`, 1 s), with every decision, every counter change and every change of verdict or deciding arm kept — the transitions are what the events are read for, and only the repeats between them are dropped. A retention purge is no longer silent either: it leaves an `aud {a:"purge", n, why:age|rows|bytes}` line, written after the deletion so it survives it. Verbose should now be ~2 MB/h where the same instrumentation *measured* ~6 — a projection from the line counts, not a device figure; the next verbose run is what confirms it. The auto-stop-from-pause decision is journalled on the way (a trip ended by `maxPause`, the way trip 6 ended, left no `stop` line at all), and `res` gains `mv`, the milliseconds of continuous movement the resume decision is actually made on — L-082 was believed to be a 100 ms defect that the `cm` counter — which the resume path never touches — could not show; T045 withdrew that finding (the resume rule was already a 5 s one), and `mv` now reports the quantity the decision is made on either way. Three honesty fixes with it (L-086): the `perm k:background` listener ignores the `AsyncLoading` that `refresh()` produces and drops a re-read that reports what the OS already reported (two identical lines per app resume); `bat` de-duplicates through a process-wide `BatteryAuditThrottle` that outlives the notifier's rebuild, so a session restart no longer writes the same reading twice, while an unchanged level still gets one line per 5-minute tick; and the stop/resume detector's pause clock is now `so` — seconds since the **stationary onset** — where it was `pd` and read like the trip's own `pau`, which counts from the pause *transition* `minPause` seconds later (schema `sv` 1 → **2**). `07fabee`'s "the configured rate exactly" is corrected rather than enforced: a sampling period is a request the OS rounds to its own rate (55.6 Hz backgrounded, 83 Hz foregrounded against a configured 50 on the Pixel; 51.4 Hz on the iPhone), so `hb` now carries `hz` and the measured rate is read as `mn / (dt / 1000)` against it. Holding the pipeline to the configured rate is a detection-and-battery decision and stays with T045. Tests 636 → 654. |
| 20 — one departure, one trip | (this change) | **L-080** (closed) | T044, the bounded half. The motion stream runs at ~50 Hz and `_onMotionData` is launched with `unawaited`, so every sample that arrived while `startRecording` was awaiting its `saveTrip` re-entered `_analyzeForTripStart` with the state machine still in `Detecting` and a detector still answering `shouldStart` — one departure, two `startRecording` calls, two database rows, and trip 5 left `active` for the next launch's recovery to close. `_activeTrip != null` could never have caught it: that field is assigned *from* the awaited result, and `hasActiveTrip` is the same guard one layer up, true only at `startTripWithId`. All three start paths now claim the departure **synchronously**, which is sufficient because neither the test nor the set can be interleaved on Dart's single thread: `TripRecorderService.startRecording` sets `_startInFlight` before its first `await`; `TripDetectionCoordinator._analyzeForTripStart` claims the same flag before the pre-trip buffer hand-off, re-checked after the detector's `await` because the guard at the top of the method only rejects samples arriving *after* the decision, not the one already parked on that future; and `startTripManually` claims before its audit line and before `await startListening()` — a double tap is otherwise indistinguishable from the coordinator's own race, and it used to write a second `trip start` line before being rejected. Everything that can throw between a claim and its release sits inside the `try`, and `startListening` clears the coordinator's flag as well: a claim released nowhere would have stopped detection for the life of the process, silently, since the early return also suppresses the `start` eval. The loser of a race is then told *which* failure it is: the recorder throws `TripAlreadyStartingError` (a `StateError`, so the existing double-start contract holds) and both callers skip their teardown on that type only. `hasActiveTrip` cannot answer that question — the rejection necessarily lands before the winner reaches `startTripWithId`, so it reads false in exactly the window it was meant to cover, and both error paths would have stopped the ride the other caller had just started; a manual tap that loses now returns quietly instead of raising a "Could not start the trip" snackbar over a trip that is starting. One rollback added as defence in depth and marked untested for the honest reason: a failure *after* `startTripWithId` would leave `_activeTrip` set (every later start rejected for the life of the process) behind a state machine saying `active` with nothing driving it, so the recorder's `catch` now unpublishes — no current path reaches it, because every await past that point already swallows its own failure. **Not device-validated**: the acceptance test is the next control run, where one departure must leave exactly one row and no `active` orphan. L-079 (walking scores as cycling) and L-081 (a 0 m trip is saved) are untouched and keep T044 open. Tests 654 → 664. |
| 21 — a ride the app has no record of | (this change) | **L-081** (half closed) | T044. `Trip.isValidTrip` asked one question — is this longer than `minTripDurationSeconds`? — and nothing looked at whether anything had been *recorded*, so the 2026-09-02 control run put two 0 m recordings in History as rides: trip 4 ran 627 s on a single fix the accuracy filter rejected, trip 6 ran 134 s without a fix at all. The same recordings were already deleted after a process kill — `TripRecoveryService.rebuildFromRoutePoints` has always refused to describe a ride from fewer than two points — so an enregistrement survived or not depending on how it ended. The getter becomes `isRideWorthKeeping(int routePointCount)`, applied by the live stop path and by recovery alike: too short, or fewer than `minTripRoutePoints` (**2**) points. Two is the code's own number, not a taste: `_recordLocation` accumulates distance only against a previous point, so a one-point recording reads 0 m whatever really happened, and 2 is the smallest count that *can* describe a movement. Still no minimum-distance rule — a 22 m ride is kept — because the question is whether the app has any record of a ride, not how far it went. The count had to be built: `_routePointBuffer.length` empties on every flush, so a cumulative `_routePointsRecorded` now feeds the decision, the `trip` line (`n`, without which a `discard` would not say what decided it) and `TripMetrics.routePointCount`, which was reporting the buffer's sawtooth to a field no UI reads. Two consequences followed the rule rather than the finding. `discarded` stopped meaning "false start": an eleven-minute ride through an urban canyon whose fixes are all rejected is discarded too, and arming `tripStartCooldownPeriodSeconds` on it would blind the detector for another 30 s just as the rider comes back into the open — the cooldown now tests the duration arm, and only that. And a *manual* stop used to end in silence, because the "trip recorded" notification is suppressed for a discarded ride (L-069): after confirming a dialog reading "0 m · 45:12" the user landed on an unchanged history with nothing to explain it, so the tracking screen now says which rule rejected the ride. **The other half of L-081 stays open**: both trips ended through a safety net rather than a decision (trip 4 by the L-074 watchdog at the full 600 s, trip 6 by `maxPause`), which is a *detection* failure of the same family as L-079 and belongs with the rest of T044/T045. Worth saying plainly, because this change makes it harder to see: such a recording used to leave two traces — an audit line and an aberrant history entry, and it is the second that got L-081 opened — where it now leaves only a `trip discard` line in an opt-in log. The next control run must count the `trip discard` lines carrying `n: 0`; if there are any, detection is still starting rides that are not there and the new rule is only hiding the result. `minTripPts` is in the header's `k` so that line stays readable three weeks later. Tests 664 → 674. |
| 22 — the sampling rate becomes a rate | (this change) | **L-086** rate leftover (closed); **L-082** withdrawn; **L-083** re-diagnosed, still open | T045. `motionDataStream` now holds itself to `PowerModeConfig.sensorSamplingRate` by dropping the surplus accelerometer samples, scheduling by slot (`nextSlot += period`) so the long-run average is the configured rate rather than drifting with every late sample. The battery arithmetic is the smaller half of the reason: 83 Hz foregrounded against a configured 50 is two thirds more coordinator passes than asked for. The larger half is that without it `sensorSamplingRate` was **decorative** — dropping from 50 Hz in `normal` to 20 Hz in `critical` only lengthened a *requested* period, and an OS free to round it to its own supported rate could hand back 55 Hz in either mode; the whole battery-mode ladder changed nothing the pipeline could feel. Nothing downstream counts samples (both windows age on wall time; the detectors count one detection per `detectionEvaluationInterval`), and `stationaryWindowMaxSamples` — 128, against ~125 at 83 Hz over 1.5 s — is now comfortably clear instead of nearly biting. **The risk this creates is in `critical` mode**, where 20 Hz becomes real for the first time: the 1.5 s window falls to ~30 samples and its Nyquist limit to 10 Hz, while `accelerationStdDev` is measuring exactly the high-frequency vibration of a rolling bike. 20 Hz has never been validated on a device, and a control run in `critical` is what would close it. `hb` gains `dr`, the drops, because `mn` is counted downstream of them and would otherwise read `hz` for ever — `(mn + dr) / (dt / 1000)` is what the OS delivered, `mn / (dt / 1000)` what the pipeline paid for, and without `dr` this change would have blinded the instrument that found L-086. Two hazards of pacing on a wall clock are handled: a forward jump (a suspension) resynchronises instead of firing a catch-up burst, and a **backward** jump — an NTP correction, which `clk` already journals past 2 s — resynchronises instead of refusing every sample until the clock catches up, which would have been minutes of silent detection death with `hb.mn` at zero and nothing saying why. Pacing on `AccelerometerEvent.timestamp` instead was considered and declined: it is a monotonic *uptime* (Android `uptimeMillis`, iOS `CMDeviceMotion`), so putting it in `AccelerometerData.timestamp` would make that field read as 1970-plus-uptime for every reader, and the batching it would guard against is not requested by the plugin (`registerListener` without `maxReportLatencyUs`). Tests 674 → 681. |
| 23 — iOS stops being suspended, and can come back | (this change) | **L-084** (closed, pending device) | T046. Two failures with one owner. (a) `_closeGpsGate` removed the only CoreLocation session in the process, so iOS suspended it 40 s later and the sensors that were supposed to re-open the gate stopped arriving — an absorbing state, with "Always" granted. The coordinator now hands a native session the **complement** of the gate: closed gate → a coarse 3 km session (cell/Wi-Fi, no GNSS) that keeps the process scheduled; open gate → nothing, geolocator's fine session has it. Stated from the gate's own state at `startListening` too, because a session that *starts* with the gate closed makes no transition and that is precisely the idle window that was being lost. (b) Nothing could relaunch a terminated app: the three APIs that can — significant-change, region monitoring, visits — are exposed by no Flutter package, so `ios/Runner/AutoRideBackgroundSession.swift` (~180 lines) owns a `CLLocationManager` of its own and `AppDelegate` re-arms it **synchronously** in `didFinishLaunchingWithOptions` from a `UserDefaults` flag, before `super` and before any Dart runs — the relaunch event reaches only a manager already monitoring when launch returns. `AutoDetectionController` arms and disarms it with `_appliedShouldListen`, the disarm being what makes iOS stop relaunching an app whose feature the user switched off. The native fixes are **never** positions: a 3 km fix in `_lastLocation` or the pre-trip buffer would cap the start confidence the way L-087's bogus 0 km/h did, so they cross the channel as `ios` audit lines and stop. Found on the way and fixed: `main.dart` instantiated the audit log, trip recovery and `AutoDetectionController` from an `addPostFrameCallback`, and a process relaunched into the background is not guaranteed to produce a frame — a relaunch that succeeded natively would have done nothing at all in Dart; only the navigation listener, a genuine UI concern, still waits for a frame. **Not device-validated**: the five-run protocol (idle survival, departure after a long stop, kill, reboot, disarm) plus a re-measure of T041 item 4 is in `tasks/T046-ios-background-survival.md`, and the decision paragraph in §6 says what was traded against the battery target. Tests 702 → 724. **Xcode/privacy checked against Apple's documentation rather than assumed**: `Info.plist` needed nothing (`UIBackgroundModes: location` was already there, and it is the only requirement for `allowsBackgroundLocationUpdates`), no entitlement or capability exists for any of the three APIs, and CoreLocation is in none of the five required-reason categories so `PrivacyInfo.xcprivacy` needed no addition. It did need a **correction**, and one that predates T046: the User Defaults reason was `1C8F.1`, the *App Group* case, in a project with no `.entitlements` file at all — a false statement in the manifest, now `CA92.1` ("only accessible to the app itself"). T046 is what made it worth finding, the native session writing `autoride.detectionArmed` being a second consumer of that category. `store-metadata/data-safety.md` §4.2 updated with the same reasoning. |
...
**Decision (2026-09-01) — L-075: a timeout is not a false start.**
The 30 s `Detecting` timeout and the 30 s post-false-start cooldown were doing the same job
badly, from opposite ends. Options considered and why the chosen one won:

1. **Drop the cooldown on timeout entirely** (chosen) rather than shorten it to ~5 s. A shorter
   cooldown is still a blind window bought with nothing: the timeout fires precisely because *no
   trip was started*, so there is no false positive to suppress and no evidence about the next
   30 s. Shortening it would only make the bug smaller. The cooldown's stated purpose — "prevents
   immediate restart after false start" — is met exactly once: a trip that started and was
   discarded, which is now the only path that arms it.
2. **Enter `Detecting` on a counted detection, not on any motion** (chosen) rather than making
   `Detecting` purely internal to the detector. Making it internal is the cleaner design but is
   not a minimal change: `TripStateMachine.startTripWithId` transitions out of `detecting` and
   nothing else, the manual start button in `AutoDetectionController` opens the phase by hand
   before starting, `hasDetectionTimedOut` is defined on it, and the tracking screen reads it.
   Gating the *entry* keeps every one of those working while removing the churn — the phase now
   opens on the same event the streak does. `startDetecting()` is idle-only, so the call is
   idempotent and is made on the confirming sample too.
3. **Keep the 30 s timeout.** Without the cooldown and the teardown it is inert bookkeeping: it
   returns the machine to idle and drops an unconfirmed streak. Removing it instead would leave
   `Detecting` latched forever on a single noisy sample.
4. **Drop the timeout's stream restart.** `_suspendListening()` + a 100 ms `startListening()` also
   closed the GPS gate, so a walking user's positions were re-acquired every 30 s. Nothing needed
   restarting: the detectors are reset explicitly and the gate re-evaluates on the next sample.

`wakelock_plus` was removed rather than wired up: both platform implementations are *screen* wake
locks tied to a foregrounded activity (`FLAG_KEEP_SCREEN_ON` / `isIdleTimerDisabled`), verified in
`~/.pub-cache`. Enabling one during a ride would keep the display on — battery cost, no sensor
benefit — and would do nothing at all in the case that matters, the app in the background, where
`flutter_background_service_android`'s `PARTIAL_WAKE_LOCK` is what actually keeps the CPU running.


**Decision (2026-09-01) — L-076: the detection phase is buffered and replayed, not re-derived.**
The gap is structural: confirmation is necessarily *late* (three consecutive detections, and the
GPS gate opens before the detector says anything), so the only way a trip can start where the ride
started is to keep what was seen while deciding. Options considered:

1. **Buffer in the coordinator and replay into the recorder** (chosen) rather than moving the
   location subscription's ownership into the recorder, or having the recorder query the
   repository for recent fixes. The coordinator is already the only owner of a detection-phase GPS
   subscription (audit #3), so the fixes cost nothing extra to keep; nothing else in the app knows
   they exist, and they are never persisted before a trip claims them. Handing them over as an
   argument keeps the recorder's contract explicit — a caller with no trustworthy history passes
   nothing and gets exactly the old behaviour.
2. **Replay through the live path** (chosen) rather than a bespoke prefix accumulator. A second
   implementation of "what counts as a route point" would drift from the first the moment either
   filter changed. `_handleLocationUpdate` is split into its guard half (active trip, not paused)
   and `_recordLocation`, the filter/accumulate half; the replay calls the latter. That also makes
   the replay independent of where `startTripWithId` sits in `_startRecording` — the state-machine
   phase is no longer part of the question.
3. **Skim to the first fix at `cyclingSpeedMin`, keeping the whole tail** (chosen) rather than
   filtering fix-by-fix on speed, or keeping the buffer whole. The gate opens on *any* movement,
   walking included, so the head of the buffer is routinely the approach to the bike: keeping it
   would put the trip's origin at the front door and add the walk to the distance. But filtering
   each fix independently would delete the legitimate slow moments a few seconds into a ride (a
   junction, a red light) and break the route into disconnected jumps. The first fix at cycling
   speed is where the ride demonstrably began; everything from there is the ride. Nothing at all
   above 8 km/h ⇒ nothing is prefixed, and the trip starts at confirmation as before.
4. **Back-date `startTime`, not just the distance.** Carrying the metres without the seconds would
   have made every trip's duration too short and its average speed too high — a worse lie than the
   one being fixed. `Trip.toMap()` writes `start_time`, so the 30 s snapshot and the final write
   carry it anyway; the extra immediate `updateTrip` exists only for the process-death window.
5. **Two bounds, 90 s and 64 points.** 90 s covers the worst case the detection phase can produce
   (30 s of `Detecting` + confirmation + the recorder's first-fix delay) with margin, and is short
   enough that nothing a rider would call "before the trip" can survive in it. 64 points is a
   guard, not the operative limit: at a 15 m distance filter and ~20 km/h a fix lands about every
   3 s, so 90 s is ~30 points. Either bound alone would be enough in normal operation; both are
   there because a stalled clock defeats the first and a chatty plugin defeats the second.

**Interaction with L-068 (sub-60 s trips are deleted).** The rule is unchanged and still applies to
the *whole* trip, prefix included — which is the behaviour that was wanted. A ride confirmed after
40 s of actual riding now arrives at `stopRecording` already carrying those 40 s, so a rider who
sets off, is detected, and stops after 25 more seconds keeps a 65 s trip instead of having a real
...
|---|---|---|---|---|---|
| L-079 | pipeline | `trip_start_detector.dart` | Walking scores as cycling, and with no GPS the score is 100 % motion | Confirmed (device) | Every `start go:true` and every aborted `detecting` window (9 `dto` in 30 min of walking in a shop, 2 starts) carries `mag` 12–15 m/s² and `gyr` 1.4–2.0 rad/s — inside the single-sample cycling fit. **Correction (T045, 2026-09-02)**: `spk` is present on **2 384** of the 6 389 evaluations, not absent on all of them. What the 2 384 say is stronger than the original claim: not one of them reached the 0.7 threshold (highest 0.53), because a fix reading 0 km/h scores `speedScore` 0 and caps the confidence at `motion × 0.6 = 0.60` — arithmetically below the threshold, not merely damped. Recomputing `motionScore` from each line's `mag`/`gyr` reproduces `c` to within 0.0006 on both groups, so this is the formula, not a correlation. The 4 005 evaluations *without* a fix are where all 21 threshold crossings and both `go:true` live. L-022's fix made the *streak* 1 Hz; the *confidence* is still a one-sample fit, and the three-layer detector (L-011) is still dead. The vehicle ride home (13 gate openings, 18:16–18:38) produced no false start: the failure is specific to walking. → **T044** |
| L-080 | pipeline · arch | coordinator `_analyzeForTripStart` / recorder `startRecording` | Two trips started from one departure (trips 5 and 6, same millisecond) | Confirmed (device) | Two `start go:true` in the same second each reached `startRecording`. The recorder's `_activeTrip != null` guard is only set after the `await saveTrip`, the state machine stays `detecting` until `startTripWithId`, and the detector keeps answering `shouldStart` for every sample inside the evaluation interval. Trip 5 got a database row and nothing else — no `st`, no stop — and is left `active` for `TripRecoveryService` (L-068) to close at the next launch. L-075's cooldown never armed: neither trip is a "false start" by its definition. → **T044**, closed by remediation row 20: both the coordinator and the recorder claim the departure synchronously, before the first `await` on the start path. |
| L-081 | pipeline | recorder stop path / L-068 rule | A trip with zero route points is saved as a ride | Confirmed (device) | Trip 4: 627 s, one fix (accuracy 100 m, dropped by `rpAcc` 50), `dist` 0, saved. Trip 6: 134 s, no fix at all, saved. L-068 deletes only trips shorter than `minTripDurationSeconds`; nothing looks at distance or point count. Both ended through safety nets rather than a decision: trip 4 by the L-074 watchdog after the full 600 s, trip 6 by `maxPause` (`pd` 299). Three pause/resume cycles on trip 4 came from `gps+vib` windows whose network fixes all read 0 km/h. → **T044**. Half closed by remediation row 21: a recording with fewer than `minTripRoutePoints` route points is deleted rather than saved, on the live stop path as it already was on the recovery path. The safety-net ending is untouched and is the detection half — it stays with T044/T045, and the next control run has to count the `trip discard` lines with `n: 0` to see it. |
| L-082 | pipeline | `trip_stop_detector.dart`, resume path | ~~Resume is decided over 5 *samples*, not 5 seconds~~ | **Withdrawn (T045, 2026-09-02) — not a defect** | The original reading was that all three `res go:true` on trip 4 carry `cm:5` while `res` is evaluated per motion sample, i.e. ~100 ms of movement. Wrong on both halves. `shouldResumeTrip` does not read `cm` at all: it decides on `state.movementStartTime` against `resumeMovementThresholdSeconds` (5 s of *continuous* movement), and that code was already in 1.0.0+9 (`git show 03e2b03:…/trip_stop_detector.dart`). `cm` is the `analyzeForTripStop` hysteresis counter, which advances once per `detectionEvaluationInterval` — measured at 1 014 ms a step in this very log — and is zeroed by any stationary reading. So `cm:5` *is* five seconds. All three resumes show cm 1→5 over 4.05 s plus the ~1 s before the first increment: ≥ 5 s of continuous movement, exactly as designed. T041 item 9 is satisfied on this evidence. The `mv` field T047 added to `res` reports the quantity the decision is actually made on and will confirm it directly on the next run. What the three resumes *do* show is a walker's pocket producing five seconds of continuous non-stationary motion — which is L-079's problem, not the resume rule's. |
| L-083 | pipeline · battery | coordinator GPS gate / `MotionStateWindow` | The gate never closes while the phone is carried on foot | Confirmed (device) | Pixel: one continuous open interval 17:16:38→18:13:03 (56 min) for 4 usable fixes; 195 `gate sched` for 15 closes; 4 058 s open in 2 h 05. Every "moving" window verdict re-arms the 30 s timer, so a walker keeps GPS on for nothing. `07fabee`'s window fixes the phone lying still (iPhone at home: 7 min open then one close, against "never" on +8) — not the phone in a pocket. Item 4 cannot be quoted from this run (verbose level). → **T045**, **still open** after T045: the finding stands and the numbers below sharpen it, but the fix is sequenced behind L-079 (see the decision paragraph in §6). Sharper evidence: the longest interval is 17:35:16→18:13:03, 38 min, entirely outside any trip, and it produced **4** fixes — all at 0 km/h and 100–200 m accuracy, i.e. network fixes, so the receiver was not even the cost it looks like. Over the whole file: 31 fixes in 2 h 05, every one of them 0 km/h. |
| L-084 | platform · pipeline | iOS background / GPS gate | iOS suspends the process the moment the gate closes, "Always" or not | **Fixed** (T046) — pending the device runs | `perm {alw:true, acc:precise}`, `fgs start plat:ios`, `app paused` 16:05:32, gate `close inactivityTimeout` 16:04:51. Heartbeats after that: 16:21:01 (`dt` 946 s), 16:36:04 (`dt` 903 s), 19:07:29 (`dt` 9 085 s = 2 h 31). No launch header in between → a **suspension**, not the 1.0.0+8 termination. The 16:36 wake (gate open, one 27 m fix at 12 km/h) lasted one tick. The whole outing is invisible on iOS. This is precisely the case the L-078 decision set aside ("a gate that closes the only subscription holding the process"): T041 item 8's iOS verdict ("While Using") was one of two causes, and this is the other. → **T046** |

### Medium

| ID | Dim | Where | Finding | Status | Evidence |
|---|---|---|---|---|---|
| L-085 | gates · battery | coordinator `_analyzeForTripStop` / `_analyzeForResume` audit emits | Verbose `win`/`stop`/`res` fire per motion sample, and the log purges its own session | Confirmed (device) | Inter-event gaps 1–12 ms; 80 103 `win` + 33 036 `stop` + 23 535 `res` = 90 % of the Pixel file for ~26 min of recording; 12 MB uncompressed for 2 h. The 20 MB byte bound (`auditMaxBytes`) purged everything before 17:05:54 **silently** (a retention purge is not an `aud` event, by the L-077 design): no launch header, no `sess start`, no `fgs`, no `perm`, and trip 4 starts mid-flight — T041 item 8 is unverifiable from this log on Android. Same defect class as the `start` emit throttled in `07fabee`, on the two paths it did not touch. → **T047**, closed by remediation row 19: the three emits are throttled and a purge now leaves an `aud {a:"purge"}` line. |

### Low

| ID | Dim | Where | Finding | Status | Evidence |
|---|---|---|---|---|---|
| L-086 | gates | audit emit sites, sensor pacing | Duplicate and mutually inconsistent emissions | Confirmed (device) | `perm k:background why:change` emitted twice, identical, on every `app resumed` (iOS ×4, Pixel ×1); two `bat` lines in the same second at every session restart; trip 6 reports `pau` 269 (recorder, from the pause transition) while the detector's `pd` reads 299 (from stationary onset) — two clocks for one pause; Pixel `hb.mn` median 1 669 per 30 s = 55.6 Hz against `hzN` 50, and 2 493 (83 Hz) once foregrounded, where `07fabee` claims "the configured rate exactly" (iPhone: 1 542 = 51.4 Hz). → **T047**, closed by remediation row 19 — except the rate itself, which is now *reported* (`hb.hz`) rather than enforced; enforcing it belongs to T045. |
...
**Decision (2026-09-02) — T045: the rate hold ships, the GPS gate waits for L-079.**

The obvious fix for L-083 is to stop letting a "moving" verdict hold the gate open — a
productivity bound (close after N minutes with no fix at `cyclingSpeedMin`), or letting GPS vote
to close as well as motion votes to open. It is not shipped here, and the reason is worth writing
down because the first reason considered was wrong.

*The wrong reason.* A fix reading 0 km/h caps the start confidence at `motion × 0.6 = 0.60`,
below the 0.7 threshold, so no evaluation that had a fix could ever start a trip — 0 of 2 384,
against 21 threshold crossings in the 4 005 without one, both `go:true` among them. It looks
exactly like "the open gate is the only thing holding the false-start rate down". It is not: a
departure is three counted detections inside `tripStartDetectionWindowSeconds`, not one crossing.
Replaying the streak machine over the same log with every `c` replaced by its motion-only value —
the gate closed everywhere — yields **the same single departure, at the same second**. Only 8 of
the 2 384 would have crossed the threshold motion-only, and they never group three-in-five-seconds.
The base rates match too (0.34 % against 0.52 %), so the gate is not selecting calmer periods.
Closing the gate would have cost nothing on this run.

*The right reason.* Closing the gate puts **100 % of evaluations on the motion-only path**, which
is precisely the condition L-079 describes as broken ("with no GPS the score is 100 % motion").
Generalising a condition we already know is the defect, before the single-sample fit is replaced,
is the wrong order — even though this particular run would not have noticed. L-083's fix therefore
follows L-079's, and the counterfactual above is recorded so nobody has to re-derive it.

*A finding on the way, and it belongs in the same queue.* `_lastLocation` is only cleared when the
gate closes or GPS errors — there is **no freshness check** before `analyzeForTripStart`, though
`stationaryGpsMaxAge` (10 s) exists and `TripStopDetector._freshSpeedKmh` applies it. Across the
38-minute interval with 4 fixes, a fix more than ten minutes old kept the start confidence pinned
at zero. So the damping described above is partly an accident of staleness rather than a
measurement. Fixing it in isolation would raise the motion-only share exactly as closing the gate
...
**Decision (2026-09-03) — T046: iOS keeps a coarse session while idle, and hands the relaunch job to CoreLocation.**

L-084 is not a bug in the gate. The gate is right, and on Android it is right for the reason
L-067 records: the foreground service holds the process whatever GPS is doing, so closing the
gate costs nothing but a receiver. On iOS there is no such holder. The location session **is**
the process's reason to be scheduled, so `_closeGpsGate` removes the last one, the OS suspends
the app within a minute, and `sensors_plus` stops delivering — which means the motion that was
supposed to re-open the gate never arrives. The state is absorbing, and "Always" does not
help: it was granted on the 2026-09-02 run and the process was still gone for 2 h 31.

*What was traded.* Three ways out were on the table.

1. **Significant-change only while idle.** Costs literally nothing — the motion coprocessor
   serves it — and it also solves the relaunch problem on its own. But it wakes the process
   about every 500 m, so a departure is noticed several minutes and up to half a kilometre
   late. That is the same failure T048 has just spent a task removing from the other end of
   the pipeline, and it would make the iPhone's "three minutes and 1.3 km late" the *designed*
   behaviour rather than a defect.
2. **A coarse session, permanently, while the gate is closed.** `kCLLocationAccuracyThree`
   `Kilometers` with a 3 km distance filter is served from cell and Wi-Fi; the GNSS receiver
   never runs. Against the < 5 %/h target the cost is not zero but it is not GPS either, and
   the process staying alive is what keeps the *sensors* — which are the actual detector —
   delivering. This is what `whereiwas` calls `keepCoarseUpdatesWhileStationary`, for the same
   reason and after the same reasoning.
3. **A time-of-day or recent-motion heuristic** switching between the two. Declined: it adds a
   rule that has to be validated on a device before it can be trusted, to save a fraction of a
   cost that is already small, on a task whose whole point is that the current behaviour is
   unpredictable.

(2) ships. The number that decides it is not battery, it is that under (1) the sensors are off
between wakes, so nothing else in the pipeline runs either — the gate is not the only thing
suspension breaks.

*What it does not buy, and where the Swift is unavoidable.* Keeping the process alive says
nothing about a process that is already dead. The only three APIs that relaunch a terminated
iOS app are `startMonitoringSignificantLocationChanges`, region monitoring and
`startMonitoringVisits`, and **geolocator exposes none of them** — so this is the one place
where the "Flutter only" constraint could not hold. Significant-change and visits are armed
for exactly as long as automatic detection is enabled, and re-armed **synchronously** in
`didFinishLaunchingWithOptions` from a `UserDefaults` flag: the event that relaunched the
process is delivered only to a manager already monitoring by the time launch returns, so
re-arming from Dart, after Riverpod has built, is too late. Region monitoring is not used —
it needs a centre to monitor, which is a decision the app has no basis to make.

*The separation that keeps T048 safe.* The native manager's fixes — coarse, significant-change
and visits alike — are **never** forwarded as positions. A 3 km fix reaching `_lastLocation`,
the pre-trip buffer or `GpsSpeedEstimator` would cap the start confidence exactly the way
L-087's bogus 0 km/h did, on a much bigger scale. They cross the channel as audit lines and
stop there, and the two managers are mutually exclusive by construction: the coordinator hands
the native session the *complement* of the gate.

*Assumed limits, stated because they are iOS's and not ours.* A user force-quitting from the
app switcher stops all background delivery until they open the app again — there is no API for
that. After a reboot nothing runs until the first unlock (data protection), and then nothing
runs until the first significant change, so a ride begun immediately after a reboot loses its
first ~500 m. Both are documented in `tasks/T046-ios-background-survival.md` rather than
worked around.

*Unvalidated.* Everything above is arithmetic and API contract. What a permanently-held coarse
session costs over a day, and whether a relaunch actually fires on this phone, are device
