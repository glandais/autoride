...
void main() {
  ...
  group('SettingsService - audit instrumentation', () {
    ...
    test(
      'turning the diagnostic log off is declared before it stops',
      () async {
        await service().updatePartial((s) => s.copyWith(auditLogEnabled: true));
        await service().updatePartial(
          (s) => s.copyWith(auditLogEnabled: false),
        );
    ...
    test(
      'the settings that change what the pipeline does are all covered',
      () async {
